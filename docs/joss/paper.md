...
Some initiatives were taken in order to overcome these difficulties. For instance, the HERMES project [@druskat2022software] allows the creation of automated workflows to gather metadata from various sources and publish software on repositories based on InvenioRDM (e.g., Zenodo). The metadata conversion utility Bolognese [@fenner2017bolognese] supports many metadata formats including CodeMeta and DataCite.
...
In this paper, we present FACILE-RS, a tool to facilitate research software metadata management and archiving. FACILE-RS helps researchers follow the FAIR principles for research software through a set of scripts, which can be easily deployed within CI/CD workflows.
